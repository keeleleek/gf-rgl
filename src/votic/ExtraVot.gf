concrete ExtraVot of ExtraVotAbs = CatVot ** 
  open ResVot, MorphoVot, Coordination, Prelude, NounVot, StructuralVot, (R = ParamX) in {
  flags coding=utf8;
  lin
    GenNP np = {
...
--    i_implicPron = mkPronoun [] "minun" "minua" "minuna" "minuun" Sg P1 ;
    ...
    PartCN cn = 
      ...
      in {
        s = table {
          NPCase Nom | NPAcc => acn.s ! NPCase ResVot.Part ;
          c => acn.s ! c
          } ; 
        a = acn.a ;
...
---- copied from VerbVot.CompAP, should be shared
    ICompAP ap = {
      s = \\agr => 
          let
            n = complNumAgr agr ;
