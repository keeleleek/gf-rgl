concrete CatVot of Cat = CommonX ** open HjkVot, ResVot, Prelude in {
...
-- Sentence
    ...
    Cl    = {s : ResVot.Tense => Anteriority => Polarity => SType => Str} ;
    ClSlash = {s : ResVot.Tense => Anteriority => Polarity => Str ; c2 : Compl} ;
    Imp   = {s : Polarity => Agr => Str} ;

-- Question

    QCl    = {s : ResVot.Tense => Anteriority => Polarity => Str} ;
...
-- Relative
    ...
    RCl   = {s : ResVot.Tense => Anteriority => Polarity => Agr => Str ; c : NPForm} ;
    RP    = {s : Number => NPForm => Str ; a : RAgr} ;

-- Verb

    VP   = ResVot.VP ;
    VPSlash = ResVot.VP ** {c2 : Compl} ; 
    Comp = {s : Agr => Str} ; 
