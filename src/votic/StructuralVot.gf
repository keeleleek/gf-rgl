concrete StructuralVot of Structural = CatVot **
  open MorphoVot, ParadigmsVot, (X = ConstructX), MakeStructuralVot, Prelude in {
...
---  one_Quant = mkDet Sg  DEPREC
  ...
  that_Quant = heavyQuant {
    s = table (MorphoVot.Number) {
          Sg => table (MorphoVot.Case) {
            c => (mkPronoun "too" "tolle" "toda" Sg P3).s ! NPCase c
            } ;
          Pl => table (MorphoVot.Case) {
...
oper
  jokuPron : MorphoVot.Number => (MorphoVot.Case) => Str =
    let
  ...
  -- TODO: maybe remove
  jokinPron : MorphoVot.Number => (MorphoVot.Case) => Str =
    table {
      Sg => table {
        Nom => "miski" ;
        Iness => "milleski" ;
  ...
  --TODO does this work?
  mikaInt : MorphoVot.Number => (MorphoVot.Case) => Str =
    let {
      mi  = mkN "mille"
    } in
    table {
  ...
  kukaInt : MorphoVot.Number => (MorphoVot.Case) => Str =
    let
  ...
  mikaanPron : MorphoVot.Number => (MorphoVot.Case) => Str = \\n,c =>
    case <n,c> of {
        <_,Nom> => "ükski" ;
        <_,Part> => "ühtegi" ;
        <_,Gen> => "ühegi" ;
  ...
  kukaanPron : MorphoVot.Number => (MorphoVot.Case) => Str =
    table {
      Sg => table {
        Nom => "keegi" ;
        Part => "kedagi" ;
...
oper
  makeNP  : N -> MorphoVot.Number -> CatVot.NP ;
  makeNP noun num = {
